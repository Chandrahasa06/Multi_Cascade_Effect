...
class TestFlowStateUpdate:
    ...
    def test_iat_sum_sq_accumulates_squares(self):
        state = FlowState.start(make_packet(timestamp_us=0))
        state.update(make_packet(timestamp_us=1000))  # iat 1000
        state.update(make_packet(timestamp_us=1500))  # iat 500
        state.update(make_packet(timestamp_us=5000))  # iat 3500
        assert state.iat_sum_sq == 1000**2 + 500**2 + 3500**2

    def test_iat_sum_sq_zero_with_no_iat(self):
        state = FlowState.start(make_packet(timestamp_us=0))
        assert state.iat_sum_sq == 0

    def test_out_of_order_timestamp_clamps_iat_sq_to_zero_too(self):
        state = FlowState.start(make_packet(timestamp_us=1000))
        state.update(make_packet(timestamp_us=900))  # earlier than prev -> iat clamped to 0
        assert state.iat_sum_sq == 0
