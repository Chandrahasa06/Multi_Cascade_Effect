...
class TestIatRegularity:
    def test_perfectly_regular_beacon_saturates_at_one(self):
        state = FlowState.start(make_packet(timestamp_us=0))
        state.update(make_packet(timestamp_us=1000))
        state.update(make_packet(timestamp_us=2000))
        state.update(make_packet(timestamp_us=3000))
        # every IAT is exactly 1000 -> zero variance -> regularity == 1.0
        assert compute_tier1_features(state)["flow_iat_regularity"].value == pytest.approx(1.0)

    def test_irregular_timing_is_well_below_one(self):
        state = FlowState.start(make_packet(timestamp_us=0))
        state.update(make_packet(timestamp_us=100))
        state.update(make_packet(timestamp_us=10_000))
        state.update(make_packet(timestamp_us=10_100))
        value = compute_tier1_features(state)["flow_iat_regularity"].value
        assert value is not None
        assert value < 0.5

    def test_undefined_below_two_iat_samples(self):
        state = FlowState.start(make_packet(timestamp_us=0))
        state.update(make_packet(timestamp_us=1000))  # only 1 IAT
        assert compute_tier1_features(state)["flow_iat_regularity"].value is None

    def test_undefined_for_single_packet_flow(self):
        state = FlowState.start(make_packet(timestamp_us=0))
        assert compute_tier1_features(state)["flow_iat_regularity"].value is None

    def test_undefined_when_all_iats_are_zero(self):
        state = FlowState.start(make_packet(timestamp_us=0))
        state.update(make_packet(timestamp_us=0))
        state.update(make_packet(timestamp_us=0))
        assert compute_tier1_features(state)["flow_iat_regularity"].value is None

    def test_bounded_in_zero_one_range(self):
        state = FlowState.start(make_packet(timestamp_us=0))
        state.update(make_packet(timestamp_us=7))
        state.update(make_packet(timestamp_us=5000))
        state.update(make_packet(timestamp_us=5003))
        state.update(make_packet(timestamp_us=999_999))
        value = compute_tier1_features(state)["flow_iat_regularity"].value
        assert value is not None
        assert 0.0 < value <= 1.0

    def test_low_confidence_on_coarse_resolution(self):
        state = FlowState.start(make_packet(timestamp_us=0, timestamp_resolution_us=60_000_000))
        state.update(make_packet(timestamp_us=1000, timestamp_resolution_us=60_000_000))
        state.update(make_packet(timestamp_us=2000, timestamp_resolution_us=60_000_000))
        assert compute_tier1_features(state)["flow_iat_regularity"].low_confidence is True
