"""A4 -- blind replication. Prompt version 2.

Changes from v1 (see STATUS.md's stop-point-2 findings): trigger-reason
feature names now use the trigger_ prefix (matching A1), adds
CALIBRATION_NOTE, and hypotheses now require a falsifiable prediction --
same reasoning as a3_hypotheses_v2, applied here since A4 generates its
own hypotheses independently in one pass.
...
PROMPT_VERSION = "a4_replication_v2"
...
(0 to 1), a PREDICTION, and which of your own claim_ids support or \
contradict it. The prediction must be a concrete, checkable statement \
about what the evidence would look like if the hypothesis were true -- \
something that could be confirmed or contradicted by measurements, not \
a restatement of the explanation. For example, for a monitoring/ \
health-check hypothesis, a real prediction is "inter-arrival times \
between this source's flows should be regular, close to a fixed \
interval" -- not "this looks like monitoring traffic". Produce between \
3 and 5 hypotheses, at least 2 of them benign.
"""
