...
#: minimum IAT sample count for flow_iat_regularity to be defined — with
#: fewer than 2 IATs there's no spread to measure at all, and a naive
#: single-sample computation would trivially read as "perfectly regular"
#: (see compute_tier1_features).
MIN_IAT_SAMPLES_FOR_REGULARITY = 2
...
def compute_tier1_features(flow: FlowState) -> Dict[str, FeatureObservation]:
    ...
    features["flow_iat_regularity"] = obs("flow_iat_regularity", _iat_regularity(flow))

    return features


def _iat_regularity(flow: FlowState) -> Optional[float]:
    """Coefficient-of-variation-style beaconing signal, derived entirely
    from O(1) integer registers (iat_sum, iat_sum_sq — a P4 switch
    accumulates x*x in a register exactly as it does x itself, no
    running mean or float ever stored). Low IAT variance relative to
    the mean is the classic signature of periodic C2 beaconing; this
    reports the *inverse* sense (high = regular) purely so it fits the
    existing high-side-anomalous fitting/threshold convention used by
    every other Tier-1 feature, with no changes needed there.

    Let S = iat_sum, Q = iat_sum_sq, n = iat_count (= total_pkt_count - 1).
    mean = S/n, variance = Q/n - mean**2 = (Q*n - S**2) / n**2, and
    CoV**2 = variance / mean**2 = (Q*n - S**2) / S**2 (the n**2 cancels)
    — squared to avoid a switch-unfriendly sqrt, still purely integer
    register arithmetic. This function reports its reciprocal-shifted
    form instead:

        regularity = S**2 / (Q * n)

    which is bounded in (0, 1] (Q*n >= S**2 always, by the same identity
    Cauchy-Schwarz gives for two vectors of length n: one all-x_i, one
    all-1s), equals CoV**2's reciprocal-plus-one (regularity =
    1 / (CoV**2 + 1)), and saturates at exactly 1.0 for a perfectly
    regular beacon (every IAT identical) — the same kind of hard-bound
    saturation FeatureKind.BOUNDED_RATIO's fitting already exists to
    handle correctly (see dataplane/fitting.py), which is why this
    feature is registered as BOUNDED_RATIO there rather than CONTINUOUS.
    """
    n = flow.total_pkt_count - 1
    if n < MIN_IAT_SAMPLES_FOR_REGULARITY:
        return None
    s = flow.iat_sum
    if s <= 0:
        return None  # all-zero IATs: no meaningful spread to characterize
    q = flow.iat_sum_sq
    denom = q * n
    if denom <= 0:
        return None
...
class Selector:
    ...
    def evaluate_features(self, features: Dict[str, FeatureObservation]) -> SelectorDecision:
        """Same threshold-crossing/escalation-rule core as :meth:`evaluate`,
        but taking an already-computed feature dict directly instead of a
        live ``flow``/``src_table`` pair.

        Exists for callers holding features that were correctly
        snapshotted at flow-closure time by someone else (e.g. a cached
        simulation's per-flow feature rows) — recomputing per-source
        features from a `SrcTable` object *after the fact*, once its
        buckets have rotated forward past that flow's closure instant,
        reproduces exactly the snapshot-timing bug fixed in
        eval/simulate.py's ``_finalize_flow`` (see its docstring). This
        method is the safe way to reuse those already-correct
        observations instead of re-deriving them incorrectly.
        """
        trigger_reasons: List[TriggerReason] = []
