...
class FlowState:
    ...
    iat_min: int = UNSET
    iat_max: int = 0
    #: sum of squared IATs, an O(1) integer register (a P4 switch can
    #: accumulate x*x in a wide integer register just as it does x itself)
    #: that lets check-time code derive a variance-based regularity
    #: measure without EVER storing a float or a running mean in the data
    #: plane — see dataplane/selector.py's flow_iat_regularity, added to
    #: test whether beaconing C2 traffic (near-constant IAT) is visible
    #: to a switch-implementable feature the way it apparently isn't to
    #: the plain mean/min/max IAT features.
    iat_sum_sq: int = 0
